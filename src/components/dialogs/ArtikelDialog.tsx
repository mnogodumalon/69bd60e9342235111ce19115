...
export function ArtikelDialog({ open, onClose, onSubmit, defaultValues, einkaufslistenList, personenList, enablePhotoScan = false, enablePhotoLocation = true }: ArtikelDialogProps) {
  ...
  async function handlePhotoScan(file: File) {
    ...
    try {
      ...
      const schema = `{\n  "artikelname": string | null, // Artikelname\n  "menge": number | null, // Menge\n  "einheit": LookupValue | null, // Einheit (select one key: "stueck" | "kaesten" | "liter" | "kilogramm" | "gramm" | "packung" | "flasche" | "dose") mapping: stueck=Stück, kaesten=Kästen, liter=Liter, kilogramm=Kilogramm, gramm=Gramm, packung=Packung, flasche=Flasche, dose=Dose\n  "status": LookupValue | null, // Status (select one key: "offen" | "gekauft") mapping: offen=Noch zu kaufen, gekauft=Gekauft\n  "einkaufsliste": string | null, // Display name from Einkaufslisten (see <available-records>)\n  "person": string | null, // Display name from Personen (see <available-records>)\n}`;
      ...
      setFields(prev => {
        ...
        const applookupKeys = new Set<string>(["einkaufsliste", "person"]);
        ...
        if (einkaufslisteName) {
          const einkaufslisteMatch = einkaufslistenList.find(r => matchName(einkaufslisteName!, [String(r.fields.listenname ?? '')]));
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="space-y-2">
            ...
            >
              ...
              <SelectContent>
                ...
                <SelectItem value="kaesten">Kästen</SelectItem>
          ...
          <div className="space-y-2">
            ...
            >
              ...
              <SelectContent>
                <SelectItem value="none">—</SelectItem>
                <SelectItem value="offen">Noch zu kaufen</SelectItem>
                <SelectItem value="gekauft">Gekauft</SelectItem>
              </SelectContent>
            </Select>
          </div>
          <div className="space-y-2">
          ...
          <div className="space-y-2">
            ...
            <Select
              value={extractRecordId(fields.person) ?? 'none'}
              onValueChange={v => setFields(f => ({ ...f, person: v === 'none' ? undefined : createRecordUrl(APP_IDS.PERSONEN, v) }))}
            >
              <SelectTrigger id="person"><SelectValue placeholder="Auswählen..." /></SelectTrigger>
              <SelectContent>
                <SelectItem value="none">—</SelectItem>
                {personenList.map(r => (
                  <SelectItem key={r.record_id} value={r.record_id}>
