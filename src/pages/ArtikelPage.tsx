...
export default function ArtikelPage() {
  ...
  async function loadData() {
    setLoading(true);
    try {
      const [mainData, einkaufslistenData, personenData] = await Promise.all([
            ...
            <TableRow className="border-b border-input">
              <TableHead className="uppercase text-xs font-semibold text-secondary-foreground tracking-wider px-6 cursor-pointer select-none hover:text-foreground transition-colors" onClick={() => toggleSort('artikelname')}>
                <span className="inline-flex items-center gap-1">
                  Artikelname
                  {sortKey === 'artikelname' ? (sortDir === 'asc' ? <IconArrowUp size={14} /> : <IconArrowDown size={14} />) : <IconArrowsUpDown size={14} className="opacity-30" />}
                </span>
              </TableHead>
